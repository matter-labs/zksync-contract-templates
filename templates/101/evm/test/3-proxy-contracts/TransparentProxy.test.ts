...
describe("Transparent Proxy Campaign", function () {
  ...
  beforeEach(async () => {
    ...
    owner = signers[0];
    addr1 = signers[1];
    const contractFactory = await hre.ethers.getContractFactory("ProxyableCrowdfundingCampaign");
    const fundingGoalInWei = ethers.parseEther("1").toString();
    const crowdfunding = await hre.upgrades.deployProxy(contractFactory, [fundingGoalInWei], { initializer: "initialize" });
    campaign = crowdfunding as unknown as ProxyableCrowdfundingCampaign;
  ...
  describe("V2", function () {
    ...
    beforeEach(async () => {
      ...
      const upgradedContract = await hre.upgrades.upgradeProxy(campaignAddress, contractV2Factory);
      

      v2campaign = upgradedContract as unknown as V2_ProxyableCrowdfundingCampaign;
