...
export const deployProxyContract = async (contractArtifactName: string, constructorArguments?: any[], options?: DeployContractOptions) => {
  ...
  const contract = await hre.zkUpgrades.deployProxy(deployer.zkWallet, artifact, constructorArguments, { initializer: "initialize" });
...
// Use "yarn hardhat verify <proxy address>" to verify all contract
